...
class RoleBasedAccess {
  ...
  elements = {
    ...
    effortEstimationNavigation: () =>
      cy.get("div").contains("Effort Estimation"),
    ...
    deleteButton: (recordLabel) =>
      this.action.xpath(
        `//span[text()="${recordLabel}"]/ancestor::td/ancestor::tr//td//span//button`,
      ),
    invoiceIcons: {
      regenerate: () => cy.get('[data-testid="CachedOutlinedIcon"]'),
      email: () => cy.get('[data-testid="EmailOutlinedIcon"]'),
      markAsPaid: () =>
        cy.get('[data-testid="CheckCircleOutlineOutlinedIcon"]'),
  ...
  clickButtonContaining(buttonText) {
    this.elements.buttonContaining(buttonText).click();
  }
  ...
  expectButtonToExist(buttonText) {
    this.elements.buttonContaining(buttonText).should("exist");
  }

  expectButtonToNotExist(buttonText) {
    this.elements.buttonContaining(buttonText).should("not.exist");
  }

  clickNavigationFor(moduleName) {
    this.elements.moduleNavigationFor(moduleName).click();
  }
  ...
  expectNavigationToNotExistFor(moduleName) {
    this.elements.moduleNavigationFor(moduleName).should("not.exist");
  }
}
