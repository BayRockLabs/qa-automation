...
class ProgrammaticTestSetup {
  constructor() {
    ...
    this.apiEndpoints = {
      ...
      timesheetManagerNotificationCount:
        "/c2c_service/ts-manager-notification-count",
  ...
  loadSetupData() {
    return cy
      .task("readFilesInDirectory", "cypress/fixtures/test-setup")
      .then((files) => {
  ...
  createSOWContract() {
    ...
    this.getAccessToken().then((accessToken) => {
      ...
      cy.fixture("test-documents/sow-contract-file.pdf", "binary")
        ...
        .then((fileBlob) => {
          const fileWithMimeType = new Blob([fileBlob], {
